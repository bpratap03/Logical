    ...
    public static class PlayerStatisticsCollectorImpl implements PlayerStatisticsCollector {
         ...
         public void putNewInnings(String player, int runs){
             ...
             if(hm.containsKey(player) )
             {
                 ...
                 hm.put(player,hm.get(player)+runs);
             }
             else
             {
             	hm.put(player,runs);
             ...
             if(hm1.containsKey(player) )
             {
                 
                 hm1.put(player,hm1.get(player)+1);
             }
             else
             {
             	hm1.put(player,1);
        ...
        public double getAverageRuns(String player){
        	double total_run=hm.get(player);
            int count=getInningsCount(player);
            return total_run/count;
        }

        @Override
        public int getInningsCount(String player){
        	int count=hm1.get(player);
        	return count;
